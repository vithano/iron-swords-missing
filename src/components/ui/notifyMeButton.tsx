...
const NotifyMeButton = ({
  ...
  const onEmailChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    validator.isEmail(event.target.value) && setEmail(event.target.value);
  };
    ...
    <div className="pt-8">
      ...
      {countdown === null ? (
        ...
        <div dir="rtl" className="pt-4">
          המייל לא הגיע? נסו שוב בעוד{" "}
          {"00:" + (countdown < 10 ? "0" : "") + countdown} שניות
        </div>
      )}
    </div>
  );
  const primaryButton = isLoading ? (
    <ButtonLoading>..שולח</ButtonLoading>
  ) : (
    <Button onClick={notifyMe}>עדכנו אותי</Button>
  );
  const BellIcon = (
    <svg
