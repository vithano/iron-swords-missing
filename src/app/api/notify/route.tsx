...
const getEmailsToNotify = async (id: string) => {
  ...
  for (const notification of notifications) {
    ...
    if (!isBlackListed) emails.push(notification.email);
  }
  return emails;
};
...
const handlePeople = async (data: any) => {
  ...
  if (oldRecord?.status !== newRecord?.status) {
    ...
    const fullName =
      ...
      (newRecord.last_name && newRecord.last_name.toLowerCase() !== "null"
        ? ` ${newRecord.last_name}`
        : "");
    if (emails.length) {
      await sendEmail({
        ...
        html: `שלום רב, היה עדכון בסטטוס של ${fullName}
                ניתן לעקוב אחרי העדכונים בקישור הבא: ${getBaseUrl()}/profile/${
                  newRecord.id
                }`,
      });
    }
