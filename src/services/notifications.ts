...
export async function addNotification(props?: Props): Promise<boolean> {
  ...
  if (!sanitizedEmail || !sanitizedNotifyId) return false;
  ...
  const isAlreadySubscribed = await supabase
    ...
    .limit(1);
  if (isAlreadySubscribed.error) return false;
  ...
  const { data = [] } = await supabase
    .from("notifications")
    .insert([{ email: sanitizedEmail, notify_id: sanitizedNotifyId }]);
  return true;
}
...
export async function fetchNotifications(
  ...
  const sanitizedEmail = validator.escape(email || "");
  const sanitizedNotifyId = validator.escape(notify_id || "");
  if (!sanitizedEmail && !sanitizedNotifyId) return [];
  const andQuery =
    sanitizedEmail && sanitizedNotifyId
      ? `and(email.eq.${sanitizedEmail},notify_id.eq.${sanitizedNotifyId})`
      : "";
  let orQuery =
    !andQuery && sanitizedEmail ? `or(email.eq.${sanitizedEmail})` : "";
  orQuery =
    !andQuery && sanitizedNotifyId
...
export async function blacklistEmail(props?: Props): Promise<boolean> {
  ...
  await supabase.from("blacklist_emails").insert([{ email: sanitizedEmail }]);
  return true;
}
